			...
			url : 'getPagingPower.do',
		  	...
		  	success:function(data){
		  		...
		  		for(x in data.listData){
		  			str+="<tr id='role"+data.listData[x].id+"' class='trcolor2'>";
		  			for(y in data.listData[x]){
		  				str+="<td>"+data.listData[x][y]+"</td>";		  				
		  			}
		  			str+="<td><button class='update"+data.listData[x].id+"' onclick='update("+data.listData[x].id+")'>修改</button> <button class='recovery"+data.listData[x].id+"' onclick='delete("+data.listData[x].id+")'>删除</button></td></tr>";
		  		}
		  		$("tbody").append(str);

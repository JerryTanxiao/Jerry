			...
			url : 'getPagingUser.do',
		  	...
		  	success:function(data){
		  		...
		  		for(x in data.listData){
		  			str+="<tr id='user"+data.listData[x].id+"' class='trcolor2'>";
		  			for(y in data.listData[x]){
		  				if(y!="del"){
		  					str+="<td>"+data.listData[x][y]+"</td>";		  				
		  				}
		  			}
		  			str+="<td><button onclick='update("+data.listData[x].id+")'>修改</button> " 
		  			+"<button onclick='recovery("+data.listData[x].id+")'>删除</button></td></tr>";
		  		}
		  		$("tbody").append(str);

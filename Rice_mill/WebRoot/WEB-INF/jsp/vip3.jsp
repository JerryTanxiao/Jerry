			...
			<div class="two_div3">	
				...
				<div class="seven_div1">
					<table>
						...
						<tbody>
							<c:forEach items="${sessionScope.pir.list }" var="r">
								<tr class="trcolor2">
									<td>${r.rId }</td>
									<td><fmt:formatDate value="${r.rTime }" pattern="yyyy年MM月dd日 HH点mm分ss秒" /></td>
									<td>${r.rOrderNumber }</td>
									<td>${r.rMoney }</td>
								</tr>
							</c:forEach>	
						</tbody>
						<tfoot>
							<tr>
								<td></td>
								<td colspan="2">
									<a href="##" onclick="page(${pir.firstPage })">首页</a>&nbsp;
									<a href="##" onclick="page(${pir.prePage })">上一页</a>&nbsp;
									第<input type="text" id="pagenum" value="${pir.pageNum }" style="width: 15px;"/>页 /
									共<span id="gongji">${pir.pages }</span>页
									<a href="##" onclick="page($('#pagenum').val())">跳转</a>
									&nbsp;<a href="##" onclick="page(${pir.nextPage })">下一页</a>&nbsp;
									<a href="##" onclick="page(${pir.lastPage })">尾页</a>
								</td>
								<td><span>共${pir.total}条记录</span></td>
							</tr>
						</tfoot>
					</table>
				</div>	
